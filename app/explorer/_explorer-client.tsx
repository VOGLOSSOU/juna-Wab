...
export default function ExplorerClient() {
  ...
  return (
    <div className="max-w-content mx-auto px-4 md:px-6 py-6 flex flex-col gap-5">
      ...
      {subscriptions.length === 0 && !loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {subscriptions.map((sub) => (
            <div key={sub.id} className="h-[268px]">
              <SubscriptionCard subscription={sub} />
            </div>
          ))}
          {loading && Array.from({ length: 8 }).map((_, i) => (
            <div key={`sk-${i}`} className="h-[268px]"><SkeletonCard /></div>
          ))}
        </div>
      )}
      ...
      {sheetOpen && (
        <>
          <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-[2px]" onClick={() => setSheetOpen(false)} />
          <div className="fixed bottom-0 left-0 right-0 z-50 md:left-auto md:right-0 md:top-0 md:bottom-0 md:w-80 bg-white md:border-l border-t md:border-t-0 border-border rounded-t-3xl md:rounded-none shadow-xl animate-slide-up md:animate-none flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-divider">
              ...
              <button onClick={() => setSheetOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-grey transition-colors text-text-secondary">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(Object.entries(SUBSCRIPTION_DURATION_LABELS) as [SubscriptionDuration, string][]).map(([key, label]) => (
                    <button key={key} onClick={() => setDuration(duration === key ? '' : key)}
                      className={`h-9 px-4 rounded-full text-sm font-medium transition-colors ${duration === key ? 'bg-primary text-white' : 'bg-surface-grey text-text-primary hover:bg-border'}`}>
                      {label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(Object.entries(SUBSCRIPTION_CATEGORY_LABELS) as [SubscriptionCategory, string][]).map(([key, label]) => (
                    <button key={key} onClick={() => setCategory(category === key ? '' : key)}
                      className={`h-9 px-4 rounded-full text-sm font-medium transition-colors ${category === key ? 'bg-primary text-white' : 'bg-surface-grey text-text-primary hover:bg-border'}`}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            <div className="px-6 py-4 border-t border-divider flex gap-3">
              <button onClick={() => { setCategory(''); setDuration('') }}
