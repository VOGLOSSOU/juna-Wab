...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  if (!sub) return {
    ...
    openGraph: { images: [{ url: '/juna-logo.png', width: 800, height: 400, alt: 'Juna' }] },
  }
  const ogTitle = `${sub.name} - Juna`
  ...
  return {
    ...
    openGraph: { title: ogTitle, description, url, siteName: 'Juna', locale: 'fr_FR', images: [{ url: image, width: 800, height: 400, alt: sub.name }], type: 'website' },
...
export default async function SubscriptionDetailPage({ params }: { params: { id: string } }) {
  ...
  const jsonLd = sub ? {
    ...
    '@type': 'Product',
    name: sub.name,
    description: sub.description ?? `Abonnement repas chez ${sub.provider?.name ?? 'un prestataire local'} sur Juna.`,
    image: sub.images?.[0] ?? 'https://junaeats.com/juna-logo.png',
    url: `https://junaeats.com/subscriptions/${params.id}`,
    brand: {
      '@type': 'Brand',
      name: sub.provider?.name ?? 'Juna',
    },
    offers: {
      '@type': 'Offer',
