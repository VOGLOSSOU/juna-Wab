      ...
      {!profile.isVerified && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-0.5">Vérifiez votre adresse email pour accéder à toutes les fonctionnalités.</p>
          </div>
          <a
            href={`/auth/verify-email?email=${encodeURIComponent(profile.email)}`}
            className="text-xs font-semibold text-amber-800 underline hover:text-amber-900 flex-shrink-0"
          >
            Vérifier
          </a>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {storeUser?.role === 'PROVIDER' && (
            <button
              ...
              className="w-full flex items-center gap-3 px-5 py-3.5 bg-primary border border-primary rounded-xl text-sm font-semibold text-white hover:bg-primary-light transition-colors"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/>
                <rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
              </svg>
              <span className="flex-1 text-left">Mon dashboard prestataire</span>
